...
# Temporary patch for newer Transformers
import torch

if not hasattr(__import__("transformers.modeling_utils"), "apply_chunking_to_forward"):
    def apply_chunking_to_forward(forward_fn, chunk_size, chunk_dim, *input_tensors):
        """Fallback for deprecated function."""
        if chunk_size > 0 and input_tensors[0].size(chunk_dim) > chunk_size:
            return torch.cat([
                forward_fn(*t) for t in torch.chunk(input_tensors[0], 
                                                    input_tensors[0].size(chunk_dim) // chunk_size + 1, 
                                                    dim=chunk_dim)
            ], dim=chunk_dim)
